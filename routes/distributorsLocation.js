...
router.put('/:id', [auth, authorize(["super_admin"])],async (req, res) => {
    ...
    if (!location) return res.status(400).send('location with the given id not found');
...
router.get('/:id', async (req, res) => {
    ...
    if (!location) return res.status(400).send('locatiom with the given id not found');
